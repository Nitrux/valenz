...
QString normalizeWeatherTemperatureUnit(const QString &value)
{
    const QString normalized = value.trimmed().toLower();
...
double normalizeWeatherCoordinate(const QVariant &value, double minValue, double maxValue, double fallback)
{
    bool ok = false;
...
int normalizeWeatherRefreshMinutes(const QVariant &value)
{
    bool ok = false;
...
QString weatherLabelFromCode(int weatherCode)
{
    switch (weatherCode)
    {
...
QString weatherLocationFromTimezone(const QString &timezoneName)
{
